...
// TOTAL DATA & TOTAL DUIT (Sesuai Filter)
$totalQuery = "SELECT COUNT(*) AS total, SUM(jumlah) AS total_duit FROM pengeluaran $where";
$totalResult = $conn->query($totalQuery);
$rowTotal = $totalResult->fetch_assoc();
$totalData = $rowTotal['total'];
$total_pengeluaran = $rowTotal['total_duit'] ?? 0;
$totalPages = ceil($totalData / $limit);

// DATA
$query = "SELECT * FROM pengeluaran $where ORDER BY tanggal DESC LIMIT $limit OFFSET $offset";
...
<body>
    <div class="container shadow-sm">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="text-danger fw-bold mb-0">Pengeluaran</h2>
            <div>
                <button type="button" class="btn btn-danger fw-bold" data-bs-toggle="modal" data-bs-target="#modalPengeluaran">
                    Tambah Pengeluaran
                </button>
        ...
        <div class="stats-card">
            <div class="row align-items-center text-center text-md-start">
                <div class="col-md-6 mb-3 mb-md-0">
                    <p class="mb-0 opacity-75">Total Pengeluaran (Filter):</p>
                    <h2 class="fw-bold">Rp <?= number_format($total_pengeluaran, 0, ',', '.') ?></h2>
                </div>
                <div class="col-md-6 text-md-end">
                    <p class="mb-0 opacity-75">Jumlah Catatan:</p>
                    <h4 class="fw-bold"><?= $totalData ?> Transaksi</h4>
                </div>
            </div>
        </div>
        ...
        <form method="GET" class="row g-2 mb-4">
            ...
            <div class="col-md-3 d-flex gap-1 ms-auto">
                ...
                <a href="pengeluaran.php" class="btn btn-light border">Reset</a>
            </div>
                        ...
                        <?php while ($row = $data->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="edit_pengeluaran.php?id=<?= $row['id'] ?>" class="btn btn-outline-warning btn-sm">Edit</a>
                                    <a href="?hapus=<?= $row['id'] ?>" class="btn btn-outline-danger btn-sm" onclick="return confirm('Yakin hapus data?')">Hapus</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                ...
                <ul class="pagination justify-content-center">
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $i ?>&search=<?= urlencode($search) ?>&bulan=<?= $bulan ?>&tahun=<?= $tahun ?>">
                                <?= $i ?>
                            </a>
                        </li>
                    <?php endfor; ?>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title fw-bold">Tambah Pengeluaran Harian</h5>
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" name="jumlah" placeholder="Contoh: 15000" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold">Keterangan</label>
                            <textarea class="form-control" name="keterangan" rows="3" placeholder="Misal: Beli token listrik atau ATK"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer bg-light">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" name="simpan" class="btn btn-danger px-4">Simpan Data</button>
                    </div>
                </form>
            </div>
        </div>
